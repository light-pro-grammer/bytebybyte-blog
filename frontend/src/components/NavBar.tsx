    ...
    <Navbar expand="md" collapseOnSelect variant="dark" bg="body" sticky="top">
      <Container>
        <Navbar.Brand as={Link} href="/" className="d-flex gap-1">
          <Image src={logo} alt="Byte By Byte Blog logo" width={30} height={30} />
          <span className={styles.brandText}>ByteByByte Blog</span>
        </Navbar.Brand>
...
function LoggedInView({ user }: LoggedInViewProps) {
  ...
  return (
    <Nav className="ms-auto">
      ...
      <Navbar.Text className="ms-md-3">{user.displayName || 'User'}!</Navbar.Text>
      <NavDropdown
...
function LoggedOutView() {
  ...
  return (
    <Nav className="ms-auto">
      ...
      <Button onClick={() => authModalsContext.showSignUpModal()} className="ms-md-2 mt-2 mt-md-0">
        Sign Up
      </Button>
    </Nav>
  );
